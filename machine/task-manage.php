...
<?php
    $m_mac_id = $_GET['id'];
    $upstatus = new DB_con();
    $sql = $upstatus ->fetchonerecord($m_mac_id);
    while($row = mysqli_fetch_array($sql)) {
?>
<div class="card p-1 my-1">
    <div class="card-body">
        <h5 class="modal-title text-center" id="exampleModalLabel">แบ่งงานช่าง<br>รหัสการแจ้งซ่อมทั่วไป ที่ #<?php echo $m_mac_id; ?></br></h5>
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $("#createRows").click(function(){
                            var tr = "<tr>";
                            tr = tr + "<input type='hidden' class='form-control form-control-sm' name='task_mac_id"+rows+"'  id='task_mac_id"+rows+"' size='5'>";
                            tr = tr + "<td><input type='text' class='form-control form-control-sm' name='m_mac_id"+rows+"' value='<?php echo $m_mac_id ; ?>' id='m_mac_id"+rows+"' size='10'></td>";
                            ...
                            tr = tr + "<td><input type='text' class='form-control form-control-sm' name='task_mac_status"+rows+"' value='<?php echo "กำลังทำงาน"; ?>' id='task_mac_status"+rows+"' size='8'></td>";
                            tr = tr + "</tr>";
